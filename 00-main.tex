...
\usepackage[nopatch=footnote,activate={true,nocompatibility}]{microtype}
...
\newcommand{\imageWithNote}[3]{%
  \begin{tikzpicture}
    % Include the image
    \node[anchor=south west, inner sep=0] (image) at (0,0) {\includegraphics[width=#1]{#3}};
    \node[anchor=south east, inner sep=2pt, fill=white] at (current bounding box.south east) {\parbox{.5cm}{#2}};
  \end{tikzpicture}%
}

\newcommand{\imageWithMoreNote}[3]{%
  \begin{tikzpicture}
    % Include the image
    \node[anchor=south west, inner sep=0] (image) at (0,0) {\includegraphics[width=#1]{#3}};
    \node[anchor=south east, inner sep=2pt, fill=white] at (current bounding box.south east) {\parbox{1.4cm}{#2}};
  \end{tikzpicture}%
}
